...
function useItem(invIdx) {
    ...
    switch (inv.id) {
        case 'medkit':
            const injured = G.crew.filter(m => m.hp && m.hp < 80);
            if (injured.length > 0) {
...
function openDistrictBattle(districtId) {
    const dist = G.districts[districtId];
    if (!dist) return;
...
function executeBattle(districtId) {
    ...
    if (!strat || !members || members.length < strat.manpowerMin) {
        showToast('至少选择' + strat.manpowerMin)！', 'error');
        return;
    }
        ...
        <div class="card">
            ...
            <div class="flex gap-8" style="flex-wrap:wrap;">
                <button class="btn-sm btn-purple" onclick="recruitMember(G);updateUI();" 
                    ${G.money >= 500 && available.length < (G.crewSlots || 10) ? '' : 'disabled'}>
                    🤝 招募 ($500)
                </button>
...
function renderDevTab() {
    ...
    let html = `
        <div class="card">
            ...
            <button class="btn-sm ${canUpg ? 'btn-gold' : 'btn-gray'}" onclick="if(canUpg){upgradeBase();updateUI();}" ${canUpg ? '' : 'disabled'}>扩建</button>
        </div>
        <div class="card">
...
function doNextDay() {
    ...
    // 随机事件（30%概率）
    if (Math.random() < 0.3 && typeof triggerRandomEvent === 'function') {
...
function calculateMissionReward(state, mission) {
    ...
    if (state) reward += (state.level || 1) * 10;
    return reward;
}
...
function getDailyIncome(state) {
    ...
    return lv * 30 + count * 20 + inf * 2 + 10;
}
